...
description: 当用户已经选定并确认一条 APP 或线索正式口播文案，需要用Mossland新配音和素材库画面制作无字幕纯配音混剪视频时使用；正式素材检索必须为hybrid，本地只负责ffmpeg合成，可接收零或一条已存在前贴。不要用于写文案、生成前贴、保留真人原声主轴、素材入库或投放发布。
---
...
素材搜索、稳定ID回查和短时取源使用当前 HTTP `onion-materials` MCP；不得读取旧本地索引、腾讯 COS 或直接对象存储。TTS、ASR和渲染仍按各自授权与执行适配接入。

## 固定流程

1. 冻结一条确认文案及其逐句内容；不修改正文，不从多条文案中替用户选择。
2. 确认业务线、音色、语速、前贴状态和所有外部/付费授权。
3. 使用用户已确认的`voice_id`和当前任务明确批准的Mossland付费调用生成最终旁白；最终旁白是正文唯一音轨，本地合成不改写正文或更换音色。
4. 依据最终音频取得真实词级/句级时间轴并人工听审；不能按字数估时生成正式计划。
...
8. 本地只渲染画面与Mossland旁白，不生成、烧录或内嵌字幕；画面不足时返回素材选择，不用黑屏、冻结帧或末帧补时。
...
- “本地合成”只指本地ffmpeg渲染；旁白仍来自Mossland。系统本地TTS产物不得进入正式或试验混剪交付。
